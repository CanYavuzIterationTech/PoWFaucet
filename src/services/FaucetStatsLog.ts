...
export class FaucetStatsLog {
  ...
  public initialize() {
    ...
    if (faucetConfig.faucetStats) {
      this.enabled = true;
      this.statsFile = resolveRelativePath(
        faucetConfig.faucetStats.logfile || "faucet-stats.log"
      );
    } else {
      this.enabled = false;
    }
  ...
  private processFaucetStats() {
    let sessions =
      ServiceManager.GetService(SessionManager).getActiveSessions();
    let idleSessCount = sessions.filter(
      (s) => !s.getSessionModuleRef("pow.client")
    ).length;
    let hashRate = 0;
    ...
    statsLog.push(
      "shares: " +
        this.statShareCount +
        " (" +
        walletManager.readableAmount(this.statShareRewards) +
        ")"
    );
    statsLog.push(
      "verify: " +
        (this.statVerifyCount - this.statVerifyMisses) +
        " (reward: " +
        walletManager.readableAmount(this.statVerifyReward) +
        ", missed: " +
        this.statVerifyMisses +
        " / -" +
        walletManager.readableAmount(this.statVerifyPenalty) +
        ")"
    );
    statsLog.push(
      "claims: " +
        this.statClaimCount +
        " (" +
        walletManager.readableAmount(this.statClaimRewards) +
        ")"
    );
